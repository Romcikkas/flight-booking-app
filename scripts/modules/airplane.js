...
import decalOfNum from './decalOfNum.js';
...
const createAirplane = (title, tourData) => {
    ...
    const elements = scheme.map((element) => {
        if (element === 'exit') {
            return createExit();
        }
        if (typeof element === 'number') {
            const blockSeat = createBlockSeat(n, element, bookingSeat);
            n = n + element;
            return blockSeat;
        };
    });
...
const airplane = (main, data, tourData) => {
    const title = `Выберите ${decalOfNum(data.length, ['место', 'места', 'мест'])}`;
